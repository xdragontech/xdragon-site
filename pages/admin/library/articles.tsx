// pages/admin/library/articles.tsx
...
export default function AdminArticlesPage(_props: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  const isLibrary = router.pathname === "/admin/library" || router.pathname === "/admin/library/prompts";
  const isArticles = router.pathname === "/admin/library/articles";
  ...
  async function loadAll() {
    ...
    try {
      const [cats, arts] = await Promise.all([
        fetchJson<ApiOkCats>("/api/admin/library/article-categories"),
        fetchJson<ApiOkArticles>(q.trim() ? `/api/admin/library/articles?q=${encodeURIComponent(q.trim())}` : "/api/admin/library/articles"),
      ]);
  ...
  async function saveCategory() {
    ...
    try {
      if (catEditing) {
        await fetchJson(`/api/admin/library/article-categories/${catEditing.id}`, {
          method: "PUT",
          body: JSON.stringify({ name }),
        });
        setMsg("Category updated.");
      } else {
        await fetchJson(`/api/admin/library/article-categories`, {
          method: "POST",
          body: JSON.stringify({ name }),
        });
  ...
  async function deleteCategory(c: ArticleCategoryRow) {
    ...
    setMsg(null);
    try {
      await fetchJson(`/api/admin/library/article-categories/${c.id}`, { method: "DELETE" });
  ...
  async function saveArticle() {
    ...
    try {
      ...
      if (editing) {
        await fetchJson(`/api/admin/library/articles/${editing.id}`, {
          method: "PUT",
          body: JSON.stringify(payload),
        });
        setMsg("Article updated.");
      } else {
        await fetchJson(`/api/admin/library/articles`, {
          method: "POST",
          body: JSON.stringify(payload),
        });
  ...
  async function deleteArticle(a: ArticleRow) {
    ...
    setMsg(null);
    try {
      await fetchJson(`/api/admin/library/articles/${a.id}`, { method: "DELETE" });
  ...
  async function setArticleStatus(a: ArticleRow, next: PromptStatus) {
    ...
    setMsg(null);
    try {
      await fetchJson(`/api/admin/library/articles/${a.id}`, {
        method: "PUT",
            ...
            <div className="rounded-2xl border border-neutral-200 bg-white p-3 shadow-sm">
              <nav className="space-y-2">
                ...
                <Link
                  ...
                  className={
                    "block w-full rounded-xl bg-neutral-900 px-4 py-2 text-sm font-semibold text-white hover:bg-neutral-800 transition-colors" +
                    (isArticles ? " ring-2 ring-neutral-900/20" : "")
                  }
                >
                  Articles
                </Link>
