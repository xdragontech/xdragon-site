...
type SettingsPageProps = {
  ...
  derivedRuntimeConfig: RuntimeStatusItem[];
...
export const getServerSideProps: GetServerSideProps<SettingsPageProps> = async (ctx) => {
  ...
  return {
    props: {
      ...
      derivedRuntimeConfig: collectDerivedRuntimeConfig(),
...
export default function SettingsPage({
  ...
  return (
    <AdminLayout title="X Dragon Command — Settings" sectionLabel="Settings" loggedInAs={loggedInAs} active="settings">
      <div className="space-y-6">
        <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
          <LibraryCardHeader
            title="Settings"
            description="Read-only runtime diagnostics for the backoffice. Raw environment variables are read on the server at request time; secrets stay masked and fingerprinted."
            actionsTop={topActions}
          />

          <div className="mt-5 rounded-2xl border border-neutral-200 bg-neutral-50 p-4 text-sm text-neutral-700">
            This page is intended to answer two different questions clearly: what environment variables are actually set in
            this runtime, and what values the app derives from them after code defaults are applied. Those are not the same
            thing.
          </div>
        </div>
        ...
        <section className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
          <div className="mb-4">
            <h2 className="text-lg font-semibold text-neutral-900">Derived Runtime Config</h2>
            <p className="mt-1 text-sm text-neutral-600">
              These are the values the app will actually use after current code defaults and runtime helpers are applied.
            </p>
          </div>

          <RuntimeGrid items={derivedRuntimeConfig} />
        </section>
      </div>
    </AdminLayout>
  );
