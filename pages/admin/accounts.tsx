...
function buildExportFields(rows: any[]): ExportField<any>[] {
  ...
  const baseSet = new Set(base.map((b) => b.header));
  ...
  for (const r of rows || []) {
    ...
    for (const k of Object.keys(r)) {
      ...
      const v = (r as any)[k];
      const t = typeof v;
      if (v == null || t === "string" || t === "number" || t === "boolean") extra.add(k);
    }
  }

  const extrasSorted = Array.from(extra).sort((a, b) => a.localeCompare(b));
  return base.concat(extrasSorted.map((k) => ({ header: k, get: (u) => (u as any)[k] ?? "" })));
}
...
export default function AdminAccountsPage(props: InferGetServerSidePropsType<typeof getServerSideProps>) {
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => {
                ...
                downloadBlob(new Blob([csv], { type: "text/csv;charset=utf-8" }), `accounts-${stamp()}.csv`);
              }}
            ...
            <button
              onClick={() => {
                const html = toXlsHtml(filtered);
                downloadBlob(new Blob([html], { type: "application/vnd.ms-excel;charset=utf-8" }), `accounts-${stamp()}.xls`);
              }}
              className="shrink-0 rounded-xl border border-neutral-300 bg-white px-3 py-2 text-sm font-semibold text-neutral-800 hover:bg-neutral-50"
            >
              Export XLS
