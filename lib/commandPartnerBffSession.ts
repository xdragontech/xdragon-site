...
type CommandPartnerBffSessionPayload = {
  token: string;
  expiresAt: number;
  issuedAt: number;
};

function commandPartnerBffCookieName() {
  return process.env.VERCEL_ENV === "preview"
    ? "__Secure-stg-command-partner-bff"
    : "__Secure-command-partner-bff";
}
...
function getCookieEncryptionKey(): Buffer {
  ...
  return crypto.createHash("sha256").update(`command-partner-bff:${secret}`).digest();
}

function encodePayload(payload: CommandPartnerBffSessionPayload) {
...
function decodePayload(value: string | null | undefined): CommandPartnerBffSessionPayload | null {
...
export function getCommandPartnerBffSession(
  req: Pick<IncomingMessage, "headers"> & { cookies?: Partial<Record<string, string>> }
) {
  return decodePayload(readCookie(req, commandPartnerBffCookieName()));
}

export function getCommandPartnerBffSessionToken(
  req: Pick<IncomingMessage, "headers"> & { cookies?: Partial<Record<string, string>> }
) {
  return getCommandPartnerBffSession(req)?.token || null;
}

export function setCommandPartnerBffSessionCookie(
...
export function clearCommandPartnerBffSessionCookie(res: Pick<ServerResponse, "getHeader" | "setHeader">) {
  appendSetCookie(
    res,
    serializeCookie(commandPartnerBffCookieName(), "", {
      maxAge: 0,
      expires: new Date(0),
    })
  );
